...
def Model(x,y_,cy,T,keep_prob,is_training):
    ...
    with tf.variable_scope('Policy'):
        ...
        acr=tf.add_n(tf.get_collection('ac'),name='acr')#/batch_size
...
class SLdata:
    def __init__(self):
        ...
        '''
        id_shuffle=list(range(self.N))
        random.shuffle(id_shuffle)
        id_shuffle=np.array(id_shuffle)
        self.Xchess=self.Xchess[id_shuffle,:,:]
        self.Xstep=self.Xstep[id_shuffle,:]
        '''
        self.tr=2000000
...
if (__name__=='__main__'):
    ...
    for i in range(1,100001):
        ...
        #print(1)
        sess.run(train_op,feed_dict={x:X_batch,y_:Xs_batch,T:T_batch,cy:cy_batch,keep_prob:0.5,is_training:True})
        #print(2)
        if (i%10==0):
            X_batch,Xs_batch,T_batch,cy_batch=X.mini_batch(batch_size,is_tr=True,is_shuffle=True)
            print(i,sess.run([loss,acr],feed_dict={x:X_batch,y_:Xs_batch,T:T_batch,cy:cy_batch,keep_prob:1.0,is_training:False}))
        if (i%1000==0):
            save_path=saver.save(sess,'rollout.ckpt')
            ac_sum=0.0
